...
import styles from '@/styles/CatalogBagsMobileWomen.module.css'
    ...
    return (
        <div>
            ...
            <div className={styles.catalogContainer}>
                        ...
                        {Array.from({length: 72}).map((_, idx) => (
                            <div key={idx} className={styles.brandCircle}>
                                <div className={styles.circle}>
                                    <Image
                                        src={`/Images New Frontend/Mobile/Women/Products/Bags/Brands/${idx + 1}.png?v=${Date.now()}`}
                                        alt="Brand Image"
                    ...
                    <div className={styles.categoriesGrid}>
                        {['Все сумки', 'Сумки на пояс', 'Рюкзаки', 'Сумки тоуты', 'Сумки хобо', 'Сумки вёдра'].map((category, idx) => (
                            <div key={idx} className={styles.categoryItem}>
                                <Image
                                    src={`/Images New Frontend/Mobile/Women/Products/Bags/Categories/${idx + 1}.png?v=${Date.now()}`}
                                    alt={category}
                                    width={700}
                                    height={700}
                    ...
                    <div className={styles.categoriesGrid4}>
                        {['Спортивные', 'Кошельки', 'Клатчи', 'Кардхолдеры', 'Косметички', 'Портфели', 'Чемоданы', 'Обложки'].map((category, idx) => (
                            <div key={idx} className={styles.categoryItem4}>
                                <Image
                                    src={`/Images New Frontend/Mobile/Women/Products/Bags/Categories/${idx + 7}.png?v=${Date.now()}`}
                                    alt={category}
                                    width={700}
                                    height={700}
                ...
                <div className={styles.centerButton}>Посмотреть все сумки</div>
            </div>
        </div>
...
export default observer(CatalogBagsDesktopWomen);